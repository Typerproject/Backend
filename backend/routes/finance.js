...
router.get('/', async function(req, res) {
  try {
    ...
    const new_code_list = codelist.filter((elem) => elem.corp_name.includes(name));
    ...
    for (let i = startdate; i <= enddate; i++) {
      const dart = await axios(`https://opendart.fss.or.kr/api/fnlttSinglAcntAll.json`, {
        params: {
      ...
      const targetAccountNames = [
        '수익(매출액)', '매출원가', '매출총이익', '판매비와 관리비', '영업이익', '당기순이익',
        '영업활동 현금흐름', '투자활동 현금흐름', '재무활동 현금흐름',
        '재고자산', '유동부채', '단기차입금', '자본금', '이익잉여금'
      ];

      const new_list = list.filter(item => targetAccountNames.includes(item.account_nm)).map((elem) => ({
